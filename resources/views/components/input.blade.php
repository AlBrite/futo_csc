@props(['type','placeholder', 'value', 'name', 'pattern', 'ng'])
...
<fieldset 
  ...
  ng-init="pattern='{{$pattern}}'; type='{{$type}}'; value = '{{$value}}';placeholder='{{$placeholder}}'; focused = false; visible = false; valid = null; init()" class="input-2">
  <legend ng-class="{'focused-legend':focused}" ng-show="focused && placeholder" for="{{$name}}_input-2" ng-bind="placeholder"></legend>
  ...
  <div class="flex w-full justify-between items-center">
    <div class="flex-1">
      ...
      type="{{$displayType}}"
      ng-class="{'pr-4':!valid || '{{$type}}'=='password', 'placeholder:text-transparent': focused}"
      name="{{$name}}" 
