@props(['type','placeholder', 'value', 'name', 'pattern','size', 'leading', 'trailing'])
...
<x-checkbox name="{{$name}}" value="{{$value}}" {{$attributes}}>{!!$placeholder!!}</x-checkbox>
@elseif ($type === 'file')
<div class="flex items-center border border-slate-400 dark:border-black rounded-md overflow-clip relative"  ng-init="filename='{{strlen($placeholder) === 0 ? 'Upload file' : $placeholder}}'">
  <input type="file" ng-model="{{$name}}" name="{{$name}}" class="h-full w-full absolute opacity-0" tabindex="0"/>
  <span class="material-symbols-rounded bg-slate-200 dark:bg-zinc-900 p-2 border-r border-slate-600 dark:border-black">upload_file</span>
  <span class="flex-1 p-2" ng-bind="filename"></span>
  
</div>
@else 
  <fieldset 
    ng-controller="InputController" ng-class="{'focused': focused}"
    ng-init="pattern='{{$pattern}}'; type='{{$type}}'; value = '{{$value}}';placeholder='{{$placeholder}}'; focused = false; visible = false; valid = null; init()" class="group input-2 input-{{$size}}">
    
    <legend class="group-focus-within:block" for="{{$name}}_input-2">{{$placeholder}}</legend>
    
    <div class="flex w-full justify-between items-center">
      @if (strlen($leading) > 0)
        <div class="flex items-center icon-sm opacity-30 border-r border-gray-500 pr-3">
          {!!$leading!!}
        </div>
        @endif
      <div class="flex-1">
        ...
        name="{{$name}}" 
        ng-placeholder="focused?'':placeholder"
        value="{{$value}}"
      ...
      @if($type == 'password')
          <div class="pl-2 pr-4">
            <span class="block" ng-click="togglePassword()">
