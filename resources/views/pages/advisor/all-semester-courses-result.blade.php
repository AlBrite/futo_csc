@php
    ...
    $courses = \App\Models\Enrollment::where('enrollments.semester', $semester)
        ...
        ->orderBy('courses.id')
        ->get(['courses.code', 'courses.units', 'courses.exam', 'courses.practical', 'courses.test', 'courses.id']);

    $students = \App\Models\Enrollment::where('enrollments.semester', $semester)
        ->where('enrollments.session', $session)
        ->groupBy('enrollments.reg_no')
        ->get(['enrollments.course_id', 'enrollments.reg_no']);
    ...
    foreach ($students as $student) {
        ...
        $results = \App\Models\Enrollment::where('enrollments.semester', $semester)
            ->join('results', function ($join) {
                $join->on('enrollments.course_id', '=', 'results.course_id')->on('enrollments.reg_no', '=', 'results.reg_no');
            })
            ->where('enrollments.session', $session)
            ->where('enrollments.reg_no', $student->reg_no)
            ->orderBy('enrollments.course_id')
            ->get(['results.reg_no', 'results.score', 'results.lab', 'results.exam', 'results.test', 'results.course_id']);
    ...
    $results = $class
        ...
        ->where('enrollments.semester', '=', $semester)
        ->orderBy('courses.id')
        ->get(['courses.code', 'results.course_id', 'results.score', 'results.lab', 'results.exam', 'results.test', 'students.reg_no']);
